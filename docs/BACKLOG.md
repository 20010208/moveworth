...
最終更新: 2026-08-15（BL-20260809-02、study-work-bg専用patch結果反映）
...
#### BL-20260809-02: Published Study validator debt（Batch 1・Batch 2・Batch 3・study-country-cz・study-work-ie・study-work-rs・study-country-ae・study-work-bg専用patch完了・現状値更新）

- 優先度: 高
- 状態: 対応中（Batch 1・Batch 2・Batch 3・study-country-cz・study-work-ie・study-work-rs・study-country-ae・study-work-bg専用patch完了、残りFAIL 25件は未着手）
- 関連領域: `study_blog_posts` / `country_sources` / `scripts/utils/study-publication-quality.ts` / `scripts/patch-study-validator-debt-batch1.ts` / `scripts/patch-study-validator-debt-batch2.ts` / `scripts/patch-study-validator-debt-batch3.ts` / `scripts/add-study-source-cz-long-term-visa.ts` / `scripts/patch-study-country-cz-validator.ts` / `scripts/patch-study-work-ie-validator.ts` / `scripts/patch-study-work-rs-validator.ts` / `scripts/patch-study-country-ae-validator.ts` / `scripts/patch-study-work-bg-validator.ts` / `scripts/check-study-production-baseline.ts` / `supabase/add_study_content_cas_rpc.sql`
- 現状（2026-08-15測定、study-work-bg専用patch production適用後の確定値）: 公開済み`study-country-*`/`study-work-*` **105件**中 **PASS 80件 / FAIL 25件**（country: PASS 45/FAIL 7、work: PASS 35/FAIL 18）。総件数が103→105へ増加しているのはBG patchとは無関係の外部要因（`study-country-hu`・`study-work-ru`がscheduled publication metadata付きで監査窓内に新たに公開されPASS判定、詳細は「13. study-work-bg validator debt 恒久記録」参照）
- **実績履歴（Batch1前 → Batch1後 → Batch2後 → Batch3後 → CZ専用patch後 → IE専用patch後 → RS専用patch後 → AE専用patch後 → BG専用patch後）**: PASS 51 → 65 → 68 → 73 → 74 → 75 → 76 → 77 → 80、FAIL 52 → 38 → 35 → 30 → 29 → 28 → 27 → 26 → 25（country: 28/23 → 36/15 → 37/14 → 42/9 → 43/8 → 43/8 → 43/8 → 44/7 → 45/7、work: 23/29 → 29/23 → 31/21 → 31/21 → 31/21 → 32/20 → 33/19 → 33/19 → 35/18）。**注意**: BG専用patch前後の総件数は103→105であり（外部公開2件の影響）、上記PASS/FAIL推移はBG修正分（PASS+1/FAIL-1）と外部公開2件分（PASS+2）が合算された数値
