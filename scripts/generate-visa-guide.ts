...
// ─── URL→ラベル変換（refUrls [label](url) 形式生成用）────────────────────────

const DOMAIN_LABEL_MAP_GUIDE: Record<string, string> = {
...
function urlToLabelGuide(url: string): string {
  try {
    ...
    if (host === "canada.ca") {
      if (/\/immigration|\/refugees|\/ircc/i.test(pathname)) return "カナダ移民・難民・市民権省（IRCC）";
      if (/\/revenue-agency|\/cra\b/i.test(pathname)) return "カナダ歳入庁（CRA）";
      return "カナダ政府（canada.ca）";
    }
    return DOMAIN_LABEL_MAP_GUIDE[host] ?? DOMAIN_LABEL_MAP_GUIDE[`www.${host}`] ?? host;
  } catch { return url; }
}

function pathSuffixGuide(url: string): string {
  try {
    const { pathname } = new URL(url);
    const skip = new Set(["en","ja","fr","ko","zh","id","th","ms","pt","es","nl",
      "en-us","en-gb","en-au","index","home","top","main","portal","english","french"]);
    const parts = pathname.split("/").filter(s => s.length > 2 && !skip.has(s.toLowerCase()));
    const seg = parts.at(-2) ?? parts.at(-1) ?? "";
    return seg.replace(/[^a-z0-9]/gi, "-").replace(/-{2,}/g, "-").replace(/^-|-$/g, "").slice(0, 20);
  } catch { return ""; }
}

function buildRefLines(urlList: string[]): string {
  const labelCounts = new Map<string, number>();
  for (const u of urlList) {
    const lbl = urlToLabelGuide(u);
    labelCounts.set(lbl, (labelCounts.get(lbl) ?? 0) + 1);
  }
  return urlList.map((u) => {
    let lbl = urlToLabelGuide(u);
    if ((labelCounts.get(lbl) ?? 0) > 1) {
      const ps = pathSuffixGuide(u);
      if (ps) lbl = `${lbl}（${ps}）`;
    }
    return `- [${lbl}](${u})`;
  }).join("\n");
...
function buildPrompt(topic: VisaGuideTopic, lang: Lang, ctx: SourceContext): string {
  ...
  const refUrls = buildRefLines(ctx.urlList);
...
締め括りの文（1〜2文）。MoveWorthシミュレーターへの誘導を含める（必ずマークダウンリンク形式 [MoveWorthシミュレーター](https://moveworthapp.com/simulate) で記載すること）。
...
Closing sentence with MoveWorth simulator link. Always use markdown link format: [MoveWorth Simulator](https://moveworthapp.com/simulate)
...
结语（1～2句）。引导至MoveWorth模拟器（必须使用Markdown链接格式：[MoveWorth模拟器](https://moveworthapp.com/simulate)）。
