...
const images = ["saily-comparison.png", "saily-how-to-use.png", "saily-what-is-esim.png"];

async function compressImages() {
  for (const name of images) {
    const { data, error } = await sb.storage.from(BUCKET).download(`Saily/${name}`);
    if (error || !data) { console.log("❌ DL失敗:", name); continue; }
    const buf = Buffer.from(await data.arrayBuffer());
    const compressed = await sharp(buf).resize({ width: 1200, withoutEnlargement: true }).png({ compressionLevel: 9, effort: 10 }).toBuffer();
    if (compressed.length >= buf.length) { console.log("⏭ スキップ:", name); continue; }
    const ratio = Math.round((1 - compressed.length / buf.length) * 100);
    const { error: upErr } = await sb.storage.from(BUCKET).upload(`Saily/${name}`, compressed, { contentType: "image/png", upsert: true });
    if (upErr) console.log("❌ UL失敗:", name, upErr.message);
    else console.log(`✅ 圧縮: ${name} ${Math.round(buf.length/1024)}KB → ${Math.round(compressed.length/1024)}KB (${ratio}%削減)`);
  }
}
...
- [A8.net – アフィリエイトプログラム](https://www.a8.net/)
...
async function run() {
  ...
  const { error } = await sb.from("blog_posts").insert({
    ...
    thumbnail: null,
    title: {
      ja: "【PR】海外旅行・留学・移住に最適なeSIM「Saily」を徹底解説【2026年版】",
      en: "【PR】Saily eSIM Review: The Best Choice for Travel, Study Abroad & Relocation 2026",
      zh: "【PR】出境旅行、留学、移居最佳eSIM「Saily」全面解析【2026年版】",
    },
    description: {
      ja: "海外旅行や留学・移住に役立つeSIMサービス「Saily」を徹底解説。150か国以上対応、セキュリティ機能搭載、24時間サポートで安心の海外通信を実現します。",
      en: "A comprehensive review of Saily eSIM — covering 150+ countries, with security features and 24/7 support. The ideal data solution for travelers, students, and expats.",
      zh: "全面介绍海外旅行eSIM服务「Saily」。覆盖150多个国家，配备安全功能和24小时支持，让您的海外通信安心无忧。",
    },
    content: {
      ja: jaContent,
      en: "See Japanese version for full content.",
      zh: "请参阅日文版本的完整内容。",
    },
    locales: null,
    pinned: false,
  ...
  if (error) { console.error("❌ 投稿失敗:", error.message); process.exit(1); }
  console.log(`\n✅ 投稿完了: ${slug}`);
}
