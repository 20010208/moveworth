/**
 * ES Target-Only Fresh Reconfirmation helper（read-only, bounded）。
 *
 * 背景:
 *   BL-20260809-02 study publication validator remediationの正式選定target
 *   `study-country-es` について、production上のfresh evidence（target row /
 *   approved sources / validator result / Reference section）を安全に
 *   再確認するための専用helper。FAIL24全件やbaseline再走査は行わない。
 *
 *   本helperは `scripts/check-study-country-ge-target.ts`（GE recon helper、
 *   Codex independent code audit PASS済み）の構造・safety patternをそのまま
 *   踏襲する。module top-levelの静的importはbare specifier
 *   （@supabase/supabase-js）とNode built-in（fs, crypto）のみに限定し、
...
// ===== 対象は ES only（他slugをCLIで切り替え可能にしない） =====
...
const EXPECTED_TARGET_ID = "d5fdd3d9-5197-42c3-95d3-5f4908cf7a9a";
...
// （Spain national visa general information、visa-es tracked precedentとexact一致）。
...
// ===== timeout / request bound定数（GE recon helperと同一値） =====
...
function heartbeat(line: string): void {
  process.stderr.write(`[es-target] ${line}\n`);
}
...
let hardTimer: ReturnType<typeof setTimeout> | null = setTimeout(() => {
  process.stderr.write("[es-target] HARD_WATCHDOG_FIRED\n");
...
async function main(): Promise<void> {
  ...
  // identity driftとしてhard fatal化する（GE recon helperのCodex監査済みpatternを踏襲）。
  if (typeof row.id !== "string" || row.id.trim().length === 0) {
    ...
    // Codex M1是正: 以前はscheduledPublishAtをreportのみ（diagnostic）で保持しており、
    // hard gateではなかった。schedule anomalyをidentity driftとして明示的にhard fatal化する。
    setFatal(
  ...
  // fresh content SHA-256（GE/IE/RS/AE/BG既存patch scriptと同一method:
  // JSON.stringify(content) の sha256 hex）
  ...
  // Codex M2是正: selected approved sourceの正規化URL一致件数・purposeを、これまでの
  // freshCandidatePresent（真偽値のみのdiagnostic）から、明示的なruntime hard gateへ
  // 引き上げる。comment/logging/expected labelだけでは不十分という指摘に対応する。
  ...
  // Codex M1/M2是正: schedule anomaly・selected source match/purposeはsetFatal()経路で
  // 既にfatal化されているが、defense-in-depthとしてここでも明示的にcross-checkする
  // （既存のidMatchesExpected/isPublished checkと同じ構造パターン）。
  const structuralOk =
    result.target !== null &&
  ...
  // validator.ok は false のままで良い（ESはまだ未修正のためFAILが正常に期待される）。
