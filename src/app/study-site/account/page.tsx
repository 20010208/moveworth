...
import {
  ArrowLeft, User, Save, Trash2, AlertTriangle, CheckCircle,
  Loader2, CreditCard, Crown, ExternalLink,
} from "lucide-react";
...
export default function StudyAccountPage() {
    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50/60 via-white to-slate-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white border border-border/60 rounded-2xl p-6 sm:p-8 shadow-sm mb-8">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            {user.plan === "premium" ? (
              ...
              <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-surface text-muted">Free</span>
            )}
          </div>

          {user.plan === "free" ? (
            <a
              href="https://www.moveworthapp.com/subscribe"
              target="_blank"
            ...
            <div>
              ...
              {portalError && <p className="text-sm text-danger mt-2">{portalError}</p>}
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-danger/30 rounded-2xl p-6 sm:p-8 shadow-sm">
          ...
          <p className="text-sm text-muted mb-4">{t("account.deleteDescription")}</p>
          ...
          {!showDeleteConfirm ? (
            ...
            <div className="bg-danger/5 border border-danger/20 rounded-xl p-4 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-danger mb-1">{t("account.deleteWarningTitle")}</p>
                  <p className="text-xs text-muted">{t("account.deleteWarningDesc")}</p>
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted mb-1.5">
